        ...
        <v-data-table
          ...
          <template
            ...
            <v-icon
                small
                disabled
                @click="eliminarProductoInventario(item.idInventario)">
              mdi-delete
            </v-icon>
          </template>
        </v-data-table>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? 'Registro de producto al inventario' : 'Registro de compra'
    },
  },
  ...
  methods: {
    ...
    guardarInformacion() {
      ...
      if (this.editedIndex > -1) {
        axios
            .put("/api/inventario/" + this.idUsuarioEditar, this.editedItem)
            .then(response => {
